...
# RESTORED: All original plugins so the C++ modules actually compile
...
src_install() {
	...
	# Delete the monolithic wrapper plugins to prevent double-registration 
	# of decoders that are already built as standalone plugins.
	rm -f "${ED}/usr/$(get_libdir)/satdump/plugins/libfirstparty_support.so"
	rm -f "${ED}/usr/$(get_libdir)/satdump/plugins/libfirstparty_loader_support.so"
}
