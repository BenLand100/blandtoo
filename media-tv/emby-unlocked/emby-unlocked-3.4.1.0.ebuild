...
DESCRIPTION="Emby media and streaming server with patches to remove nags."
HOMEPAGE="http://emby.media/ https://github.com/nvllsvm/emby-unlocked/"
KEYWORDS="-* ~amd64 ~x86"
SRC_URI="https://github.com/MediaBrowser/Emby/archive/${PV}.tar.gz -> server-${PV}.tar.gz https://github.com/nvllsvm/emby-unlocked/archive/${PV}.tar.gz -> unlock-${PV}.tar.gz"
SLOT="0"
LICENSE="GPL-2"
IUSE=""
...
src_unpack() {
	einfo ${A}
	unpack unlock-${PV}.tar.gz
	mv emby-unlocked-${PV} unlock
	unpack server-${PV}.tar.gz
	mv Emby-${PV} emby-unlocked-${PV}
}
...
src_prepare() {
	...
	einfo "unlocking emby!"
	patch -N -p1 -r - Emby.Server.Implementations/Security/PluginSecurityManager.cs < ../unlock/patches/PluginSecurityManager.cs.patch
	cp ../unlock/replacements/connectionmanager.js MediaBrowser.WebDashboard/dashboard-ui/bower_components/emby-apiclient/connectionmanager.js 
}
